...
export default class Home extends Component {
    ...
    componentDidMount() {
        axios.get(`https://api.vife.dev/stock-management/api/product`)
        .then(res => {
    ...
    if (this.state.loading != true){
        if (this.state.produtos) {
            var lista = this.state.produtos.map(function (data){
                return (
                    <tr>
                        <td className="text-center">{data.pro_id}</td>
                        <td>{data.pro_name}</td>
                        <td>{data.pro_description}</td>
                        <td>{data.pro_category}</td>
                        <td className="text-right">R$ {data.pro_price}</td>
        ...
        <>
                    ...
                    <div className="collapse navbar-collapse" id="navbarNav">
                        <ul className="navbar-nav">
                            ...
                            <li className="nav-item">
                                <Link to="cadastro-cliente"> <a className="nav-link" style={{color:"#fff"}}>Cadastro De Cleinte</a></Link>
                            </li>
                            <li className="nav-item">
                                <Link to="cadastro-produto"> <a className="nav-link" style={{color:"#fff"}}>Cadastro De Produto</a></Link>
                            </li>
            ...
            <div style={{marginTop: 150, marginLeft: "35%"}}>
                <button class="btn btn-primary btn-fab btn-fab-mini btn-round" style={{width: 100, height: 100, fontSize: 50, marginRight: 100}}>
                    <Link to="/cadastro-produto"><i class="material-icons" style={{fontSize: 50, marginTop: 35, color:"#fff"}}>add</i></Link>
                </button>
                <button class="btn btn-primary btn-fab btn-fab-mini btn-round" style={{width: 100, height: 100, fontSize: 50, marginRight: 100}}>
                ...
                <div className="card">
                    <div className="card-body">
                        <table className="table">
                            ...
                            <tbody>
                                {this.state.loading ? <div><p align="center">Carregando</p></div> : lista}
                            </tbody>
                        </table>
                    </div>
                </div>
